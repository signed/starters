...
const explicitCustomDefaultTimeout = (defaultTimeout: number): Global.It => {
  const slowTest = (testName: Global.TestName, fn: Global.TestFn, timeout?: number): void =>
    _addSlowTest(undefined, testName, fn, timeout)
  const skip = (testName: Global.TestName, fn: Global.TestFn, timeout?: number): void =>
    _addSlowTest('skip', testName, fn, timeout)
  const only = (testName: Global.TestName, fn: Global.TestFn, timeout?: number): void =>
    _addSlowTest('only', testName, fn, timeout)

  slowTest.todo = (testName: Global.TestName, ...rest: Array<any>): void => {
    if (rest.length > 0) {
      throw new ErrorWithStack('Todo must be called with only a description.', slowTest.todo)
    }
    return _addSlowTest('todo', testName)
  }

  const _addSlowTest: AddSlowTest = (
