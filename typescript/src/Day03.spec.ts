...
type Move = (position: Coordinate) => Coordinate;

const moves = new Map<string, Move>();

const right: Move = (position => ({
  x: position.x + 1,
  y: position.y
}));
const left: Move = (position => ({
  x: position.x - 1,
  y: position.y
}));

const up: Move = (position => ({
  x: position.x,
  y: position.y + 1
}));
const down: Move = (position => ({
  x: position.x,
  y: position.y - 1
}));
...
function lineCoordinates(parsed: [string, number][]) {
  let position: Coordinate = { x: 0, y: 0 };
  const coordinates: Coordinate[] = [position];
  parsed.forEach(blub => {
    const [direction, steps] = blub;
    const move = moves.get(direction)!;
